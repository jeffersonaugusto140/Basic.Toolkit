...
namespace Basic.Toolkit.BTCrypt
{
    public class Crypt : ICrypt
    {
        ...
        string ICrypt.Encrypt(string texto)
        {
            byte[] plainByte = Encoding.UTF8.GetBytes(texto);
            byte[] keyByte = GetArrayKey();
        ...
        string ICrypt.Decrypt(string pTextoCriptografado)
        {
            // Converte a base 64 string em num array de bytes
            byte[] cryptoByte = Convert.FromBase64String(pTextoCriptografado);
            byte[] keyByte = GetArrayKey();
            // Seta a chave privada
            _symmetricAlgorithm.Key = keyByte;
            SetIV();
